...
// ── Startup: log whether the Gemini key was loaded ──
const _chatGeminiKeyPresent = !!Deno.env.get("GEMINI_API_KEY");
console.log("[ai-chat] Startup check — GEMINI_API_KEY:", _chatGeminiKeyPresent ? "LOADED" : "MISSING (using knowledge-based fallback)");
...
function findSchemeMatch(text: string): { summary: string; eligibility: string; application: string } | null {
  ...
  // Check for generic scheme questions
  if (lower.includes("scheme") || lower.includes("yojana") || lower.includes("subsidy") || lower.includes("benefit") || lower.includes("government") || lower.includes("apply for")) {
...
function knowledgeBasedReply(userText: string, context?: ChatRequest["context"]): string {
  ...
  if (text.includes("scheme") || text.includes("yojana") || text.includes("subsidy") || text.includes("government") || text.includes("benefit")) {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log("[ai-chat] GEMINI_API_KEY present:", !!geminiApiKey, "| length:", geminiApiKey?.length ?? 0);
    ...
    console.log("[ai-chat] Using Gemini model:", geminiModel);
    ...
    try {
      ...
      console.error("[ai-chat] Gemini fetch failed:", fetchErr);
      return new Response(JSON.stringify({ reply: "I couldn't connect to the AI service right now. Please check your internet and try again, or call the Kisan helpline at 1800-180-1551." }), {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!geminiResp.ok) {
      ...
      console.error("[ai-chat] Gemini API error:", geminiResp.status, errText);
      const reply = geminiResp.status === 429
        ? "I'm getting too many questions right now. Please wait a moment and try again."
        : geminiResp.status === 404
          ? "The AI model is currently unavailable. The app administrator needs to update the model version. Please call the Kisan helpline at 1800-180-1551 for help."
    ...
    if (!replyText) {
      console.error("[ai-chat] Gemini returned empty response:", JSON.stringify(geminiData).slice(0, 500));
      return new Response(JSON.stringify({ reply: "I couldn't generate a response. Please try rephrasing your question, or call the Kisan helpline at 1800-180-1551." }), {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
