...
export function ChatAssistant({ open, onClose, context }: ChatAssistantProps) {
  ...
  async function sendMessage(text: string) {
    ...
    try {
      ...
      const assistantMsg: ChatMessage = {
        id: `a-${Date.now()}`,
        role: 'assistant',
        text: data.reply || t.chatErrorProcess,
      };
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-forest-950/30 backdrop-blur-sm animate-fade-in" onClick={onClose}>
      <div
        className="absolute bottom-0 left-0 right-0 mx-auto flex h-[85vh] max-w-3xl flex-col rounded-t-3xl bg-forest-50 shadow-2xl animate-slide-up"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-forest-100 bg-white px-4 py-3 rounded-t-3xl">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-forest-600 text-white">
              <Bot size={22} />
            </div>
            <div>
              <h2 className="font-display text-lg font-extrabold text-forest-900">{t.chatTitle}</h2>
              <p className="text-xs text-forest-500">{t.chatSubtitle}</p>
            </div>
          </div>
          <button onClick={onClose} className="rounded-full p-2.5 text-forest-600 hover:bg-forest-50" aria-label={t.chatClose}>
            <X size={22} />
          </button>
        </div>
        ...
        <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto px-4 py-4">
              ...
              <div
                className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm leading-6 ${
                  msg.role === 'user'
                    ...
                    : 'bg-white text-forest-800 shadow-sm border border-forest-100 rounded-bl-md'
                }`}
              >
                {msg.text.split('\n').map((line, i) => (
                  <span key={i}>
          ...
          {loading && (
            <div className="flex justify-start">
              <div className="flex items-center gap-1.5 rounded-2xl rounded-bl-md bg-white px-4 py-3 shadow-sm border border-forest-100">
          ...
          {/* Suggestion chips when only the welcome/context message exists */}
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              {suggestions.map((s) => (
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-full border border-forest-200 bg-white px-3 py-2 text-xs font-semibold text-forest-700 hover:bg-forest-50"
                >
        ...
        <div className="border-t border-forest-100 bg-white px-4 py-3 safe-bottom">
          ...
          >
            <input
              ...
              className="flex-1 rounded-xl border border-forest-200 bg-forest-50 px-4 py-3 text-sm text-forest-900 placeholder:text-forest-400 focus:outline-none focus:ring-2 focus:ring-forest-400 focus:border-transparent"
              disabled={loading}
            />
            <button
              type="submit"
              disabled={!input.trim() || loading}
              className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-forest-600 text-white hover:bg-forest-700 disabled:opacity-40 disabled:cursor-not-allowed"
