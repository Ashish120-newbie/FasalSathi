...
export function SchemeDetailModal({ scheme, isBookmarked, onToggleBookmark, onClose }: SchemeDetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end bg-forest-950/40 backdrop-blur-sm sm:items-center sm:justify-center" onClick={onClose}>
      <div className="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl sm:rounded-3xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between gap-3">
          <div className="flex gap-3">
            <div className="rounded-xl bg-moss-100 p-2.5 text-forest-700"><Bookmark size={22} /></div>
            <div>
              <span className="chip bg-moss-100 px-2 py-1 text-xs text-forest-700">{categoryLabel(scheme.category)}</span>
              <h2 className="mt-2 font-display text-xl font-extrabold leading-6 text-forest-900">{scheme.name}</h2>
              <p className="mt-1 text-xs font-semibold text-forest-500">{scheme.ministry}</p>
            </div>
          </div>
          <button onClick={onClose} className="rounded-full p-2 text-forest-500 hover:bg-forest-50" aria-label="Close details"><X size={21} /></button>
        </div>

        <p className="mt-4 text-sm leading-6 text-forest-700">{scheme.description}</p>

        <div className="mt-4 flex flex-wrap gap-1.5">
          {scheme.applicableStates.map((s) => <span key={s} className="chip bg-white px-2 py-1 text-xs text-forest-600"><MapPin size={12} /> {s}</span>)}
          {scheme.eligibleCrops === 'all'
            ? <span className="chip bg-white px-2 py-1 text-xs text-forest-600"><Sprout size={12} /> All Crops</span>
            : (scheme.eligibleCrops as string[]).map((c) => <span key={c} className="chip bg-white px-2 py-1 text-xs text-forest-600">{cropById(c as never).emoji} {cropName(c as never, lang)}</span>)}
        </div>
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div className="rounded-xl border border-forest-100 p-4">
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              {scheme.eligibleFarmerCategories.map((fc) => <span key={fc} className="chip bg-moss-50 px-2 py-1 text-xs text-forest-700">{farmerCategoryLabel(fc)}</span>)}
            </div>
          </div>
          <div className="rounded-xl border border-forest-100 p-4">
        ...
        <div className="mt-4 rounded-xl border border-forest-100 p-4">
          ...
          <ol className="mt-2 space-y-2">
            {scheme.applicationProcess.map((step, i) => <li key={step} className="flex gap-3 text-sm leading-5 text-forest-800"><span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-forest-100 text-xs font-extrabold text-forest-700">{i + 1}</span> {step}</li>)}
          </ol>
        </div>

        <div className="mt-4 flex items-center justify-between rounded-xl bg-forest-50 p-3">
          <div>
          ...
          {scheme.isActive
            ? <span className="chip bg-success-100 px-2 py-1 text-xs text-success-800"><CheckCircle2 size={13} /> Active</span>
            : <span className="chip bg-error-100 px-2 py-1 text-xs text-error-800">Inactive</span>}
        </div>
        ...
        <div className="mt-5 flex gap-3">
          <button
            ...
            className={`flex flex-1 items-center justify-center gap-2 rounded-xl px-4 py-3.5 text-sm font-bold transition-all ${isBookmarked ? 'bg-forest-600 text-white hover:bg-forest-700' : 'border border-forest-200 bg-white text-forest-700 hover:bg-forest-50'}`}
          >
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-amber-400 px-4 py-3.5 text-sm font-bold text-amber-950 hover:bg-amber-500"
          >
