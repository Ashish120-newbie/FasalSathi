...
export class DiagnosisError extends Error {
  code: string;
  constructor(message: string, code: string = 'DIAGNOSIS_FAILED') {
    super(message);
...
export async function classifyCropImage(
  ...
  if (!response.ok) {
    const errorBody: AIErrorResponse = await response.json().catch(() => ({ error: '', code: 'UNKNOWN' }));
    const code = errorBody.code || 'DIAGNOSIS_FAILED';
    return { error: new DiagnosisError(errorBody.error || code, code) };
  }
